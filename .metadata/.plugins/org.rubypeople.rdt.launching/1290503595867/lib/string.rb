...
---------
     ...
     collect, count, cycle, detect, drop, drop_while, each_cons,
     each_slice, each_with_index, entries, enum_cons, enum_slice,
     enum_with_index, find, find_all, find_index, first, grep, group_by,
     include?, inject, map, max, max_by, member?, min, min_by, minmax,
     minmax_by, none?, one?, partition, reduce, reject, reverse_each,
     select, sort, sort_by, take, take_while, to_a, to_json, to_set,
     zip), Literal(to_ntriple, xsd_type)
...
-----------------
     ...
     block_scanf, bytes, bytesize, capitalize, capitalize!, casecmp,
     center, chars, chomp, chomp!, chop, chop!, concat, count, crypt,
     delete, delete!, downcase, downcase!, dump, each, each_byte,
     each_char, each_line, empty?, end_regexp, end_with?, ends_with?,
     eql?, expand_ch_hash, ext, getbyte, gsub, gsub!, hash, hex,
     include?, index, initialize_copy, insert, inspect, intern,
     is_binary_data?, is_complex_yaml?, iseuc, issjis, isutf8, jcount,
     jlength, jsize, kconv, length, lines, ljust, lstrip, lstrip!,
     match, mbchar?, next, next!, oct, original_succ, original_succ!,
     partition, pathmap, pathmap_explode, pathmap_partial,
     pathmap_replace, replace, reverse, reverse!, rindex, rjust,
     rpartition, rstrip, rstrip!, scan, scanf, shellescape, shellsplit,
     size, slice, slice!, split, squeeze, squeeze!, start_with?,
     starts_with?, strip, strip!, sub, sub!, succ, succ!, sum, swapcase,
     swapcase!, to_blob, to_date_lex, to_date_time_lex, to_f, to_i,
     to_i_lex, to_s, to_str, to_sym, to_time_lex, to_yaml, toeuc, tojis,
     tosjis, toutf16, toutf8, tr, tr!, tr_s, tr_s!, unpack, upcase,
     upcase!, upto
...
class String < Object
  ...
  #      an integer base _base_ (between 2 and 36). Extraneous characters
  ...
  #      block is given, it iterates over each byte in the string.
  # 
  ...
  #      split into paragraphs delimited by multiple successive newlines.
  # 
  #         print "Example one\n"
  #         "hello\nworld".each {|s| p s}
  #         print "Example two\n"
  ...
  # ------------------------------------------------------------------------
  #      Searches _sep_ in the string from the end of the string, and
  #      returns the part before it, the _sep_, and the part after it. If
  #      _sep_ is not found, returns two empty strings and _str_.
  # 
  ...
  #      +String#succ+ method is used to generate each value. If optional
  #      second argument exclusive is omitted or is +false+, the last value
  ...
  # ------------------------------------------------------------------------
  #      Returns true if _str_ ends with the suffix given.
  # 
