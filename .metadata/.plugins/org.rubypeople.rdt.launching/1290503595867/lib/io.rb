...
---------
     ...
     to_a, to_json, to_set, zip), File::Constants()
...
Instance methods:
-----------------
     <<, binmode, block_scanf, bytes, chars, close, close_read,
     close_write, closed?, each, each_byte, each_char, each_line, eof,
     eof?, fcntl, fileno, flush, fsync, getbyte, getc, gets, inspect,
     ioctl, isatty, lineno, lineno=, lines, open, orig_close, orig_open,
     pid, pos, pos=, print, printf, putc, puts, read, read_nonblock,
     readbyte, readbytes, readchar, readline, readlines, readpartial,
     reopen, rewind, scanf, seek, soak_up_spaces, stat, sync, sync=,
     sysread, sysseek, syswrite, tell, to_i, to_io, tty?, ungetc, write,
     write_nonblock
...
class IO < Object
  ...
  # ------------------------------------------------------------------------
  #      Calls the given block once for each character in _ios_, passing the
  #      character as an argument. The stream must be opened for reading or
  #      an +IOError+ will be raised. Multibyte characters are dealt with
  #      according to $KCODE.
  # 
  #         f = File.new("testfile")
  ...
  # ------------------------------------------------------------------------
  #      Returns an enumerator that gives each byte (0..255) in _ios_. The
