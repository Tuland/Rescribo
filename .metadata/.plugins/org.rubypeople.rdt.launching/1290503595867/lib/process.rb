...
Class methods:
--------------
     abort, detach, egid, egid=, euid, euid=, exec, exit, exit!, fork,
     getpgid, getpgrp, getpriority, getrlimit, gid, gid=, groups,
     groups=, initgroups, kill, maxgroups, maxgroups=, pid, ppid,
     setpgid, setpgrp, setpriority, setrlimit, setsid, times, uid, uid=,
     wait, wait2, waitall, waitpid, waitpid2
...
module Process
  ...
  #      when it terminates, so you can use +Thread#join+ to know the
  #      result. If specified _pid_ is not a valid child process ID, the
